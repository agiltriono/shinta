...
module.exports.execute = async function(interaction, client, userId, args) {
  ...
  var cc = db.cc
  cc[cc.findIndex(c=>c.name === commandName)].allow_vc = cc[cc.findIndex(c=>c.name === commandName)].allow_vc == true ? false : true;
  ...
  await interaction.message.reply(Object.assign({},embeds(command.allow_vc == true ? `✅ Custom command **${commandName}** tampil di Voice Chat.` : `❎ Custom command **${commandName}** tidak akan tampil di Voice Chat.`), {components: [dismis]}))
}

...
module.exports.execute = async function(interaction, client) {
  ...
  if (interaction.customId.includes("imut_vc_selectmenu_")) {
    ...
    const oldcomp = interaction.message.components
    ...
    let result = isTrusted.filter(u=>u != value[0])

    await guild.members.cache.get(value[0]).voice.disconnect()

    if (permit) await permit.delete();
    if (isTrusted.inculdes(value[0])) await client.db.update([interaction.guild.id, "voice", "temp", voiceChannel.id], {trust:result.toString()});

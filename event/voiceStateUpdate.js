...
module.exports = {
  ...
  async execute(oldState, newState, client) {
    ...
    return db.child(guild.id).once("value", async (server) => {
      const vc = server.child("voice")
      const mainChannel = vc.child("creator").val()
      const temp = vc.child("temp")
      const game = vc.child("game")
...
////////////// GAMING AREA //////////////
// NEW User Join Temporary Voice Channel
      if(!oldState.channelID && game.child(newChannelId).exists()) {
        const channel = await guild.channels.resolve(newChannelId)
        const owner = game.child(newChannelId).child("owner").val()
        if (member.user.id != owner) await channel.permissionOverwrites.create(member.user.id, {
          "SEND_MESSAGES": true,
          "READ_MESSAGE_HISTORY": true,
          "ADD_REACTIONS": true,
...
// User Leave Gaming channel
      if (oldState.channelId && oldChannelId != newChannelId && game.child(oldChannelId).exists()) {
        const memberCount = oldState.channel.members.filter(member=> !member.user.bot).size
        if (memberCount === 0) {
          // Delete the channel
          await oldChannel.delete();
